...
{
    public class IterationReport : PrintDocument
    {
        ...
        public IterationReport(Iteration iteration)
        {
        ...
        private void PrintReportHeader()
        {
            ...
            string reportHeader = "Iteration Report for ";
        ...
        private void PrintIterationTasks()
        {

            // Print all the developer tasks within an iteration
            List<IterationTask> tasks = DBInterface.GetTasksForIteration(iteration.ID);
            foreach (IterationTask task in tasks)
            {
                // Add task name
                PrintParagraphHeader(task.Task.TaskName);

                // Add task description
                PrintParagraph(task.Task.Description);

                // Add Other Developers
                List<IterationTask> otherIterationTasks = DBInterface.GetTasksForIteration(iteration.ID);
                if (otherIterationTasks.Any())
                {
                    PrintParagraphWithHeader("Other Developers: ", GetIterationText(otherIterationTasks));
                }

                if (task.PlannedCompletionDate == null)
                {
                ...
                else
                {
                    ...
                    PrintParagraphWithHeader("Completed: ", String.Format(dateFormat, iteration.EndDate));
                }
        ...
        private string GetIterationText(List<IterationTask> iterationTasks)
        {
            // String all the developer names together, separated by commas
            string text = "";
            if (iterationTasks.Any())
            {
                foreach (IterationTask iterationTask in iterationTasks)
                {
                    if (!text.Equals(""))
                    {
                        text += ", ";
